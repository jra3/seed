...
fbr() {
        ...
        # Filter out branches with '+' prefix (worktree branches)
...
fwt() {
        ...
        # Get worktree list, extract directory path and branch info
...
# SSH agent and keys
# Use macOS Keychain integration for SSH agent
if [[ "$OSTYPE" == "darwin"* ]]; then
    # Use macOS ssh-agent with keychain integration
    export SSH_AUTH_SOCK=~/Library/Group\ Containers/2BUA8C4S2C.com.1password/t/agent.sock

    # Fallback to system SSH agent if 1Password is not available
    if [[ ! -S "$SSH_AUTH_SOCK" ]]; then
        export SSH_AUTH_SOCK=/private/tmp/com.apple.launchd.*/Listeners
        # If still no socket, check for the standard macOS SSH agent
        if [[ ! -S "$SSH_AUTH_SOCK" ]]; then
            # Use the built-in macOS SSH agent
            export SSH_AUTH_SOCK=$(launchctl getenv SSH_AUTH_SOCK 2>/dev/null)
        fi
    fi
else
    # Non-macOS systems: start ssh-agent if needed
    if [[ -z "$SSH_AUTH_SOCK" ]]; then
        eval "$(ssh-agent -s)" > /dev/null
    fi
fi

# Function to add SSH keys with better error handling
add_ssh_keys() {
    local keys=(
        ~/.ssh/id_ed25519
        ~/.ssh/id_rsa
    ...
    for key in "${keys[@]}"; do
        if [[ -f "$key" ]]; then
            ...
            ssh-add -l 2>/dev/null | grep -q "$(ssh-keygen -lf "$key" 2>/dev/null | awk '{print $2}')" || {
                # Add key quietly, using keychain on macOS
                if [[ "$OSTYPE" == "darwin"* ]]; then
...
# Only add keys if we have a working SSH agent
if [[ -S "$SSH_AUTH_SOCK" ]]; then
    # Check if any keys are loaded, if not, try to add them
    ssh-add -l &>/dev/null
    if [[ $? -eq 1 ]]; then
...
# pnpm
export PNPM_HOME="/Users/john/.local/share/pnpm"
case ":$PATH:" in
  *":$PNPM_HOME:"*) ;;
